# Catalyze Wedge — Prototype Build Brief (v6)
...
v6 reflects the prototype as actually built. Key differences from v5:

- All four modules are built and functional (v5 had only 2)
- "AI as chaser in chief" is implemented — the AI acts autonomously without CEO approval
- Topics with owners auto-create goals in Goal Setting
- Auto check-ins send without CEO intervention; CEO sees results, not prompts
- Auto information gathering runs on page load for flagged topics
- Emails are signed "Catalyze for Thomas" (AI transparency)
- Delete meeting cascades to related topics, goals, and commitments
- Shared config file (`catalyze-config.js`) centralises company context, API key helpers, and design tokens
- Navigation is handled by `index.html` via iframe — modules do not have their own sidebars
...
Defined in `catalyze-config.js` as `COMPANY_CONTEXT`. Injected into every system prompt.
...
├── catalyze-config.js       # Shared config — COMPANY_CONTEXT, COLORS, AVATAR_COLORS, getApiKey(), setApiKey(), maskApiKey()
├── server.js                # Local dev server (Node.js, port 3000)
└── Catalyze_Wedge_Prototype_Brief_v5.md  # Previous brief (kept for reference)
```
...
Exports:
- `COMPANY_CONTEXT` — string injected as `system:` in every API call
...
Ten keys, all prefixed `catalyze_`:
...
| `catalyze_backlog_topics` | Meeting Capture → Topic Backlog | Array of topics auto-extracted from meetings |
| `catalyze_goals` | Goal Setting | Array of all goals (hardcoded defaults merged with auto-created). Auto-created goals have `autoCreated: true` — this flag is used by the merge logic to avoid replacing hardcoded goals, and by the delete-meeting cascade to identify which goals to remove. |
...
1. Meeting capture (`meeting_capture.html`)
2. Topic backlog (`topic_backlog.html`)
3. Goal setting (`goal_setting.html`)
4. Meeting prep (`meeting_prep.html`)
...
- One card per decision moment: flag (No decision / No owner / No deadline), topic, remediation
- "→ View in backlog" link — sends postMessage to navigate to topic_backlog.html with ?highlight=
...
- topics: 3-5 objects with { title, category, priority, owner, forum, rationale, assignmentContext, scorecard }
- decisionFlags: 2-3 objects with { topic, flagType, remediation }
...
**Near term**
- Eliminate remaining CEO approval frictions (auto-send action plans, auto-apply KR updates)
- Smartphone/mobile layout (design partner feedback: "indispensable")
- Simulated CRM data lookup during meeting analysis ("Technic Pro margin per CRM: 4%")

**Medium term**
- Real email integration (SendGrid or similar) for check-ins and action plans
